...
let evar_filtered_context_for_naming_instance evi =
...
let new_pure_evar ?(src=default_source) ?(filter = Filter.identity) ~relevance
  ...
  ?name ?(typeclass_candidate = false) ?(rrpat = false) sign ?(sign_for_naming_instance=sign) evd typ =
  let evi = {
    evar_hyps = sign;
    evar_hyps_for_printing = sign_for_naming_instance;
