...
let push_rel_decl_to_named_context
  ~hypnaming
  sigma decl ((subst, avoid, (nc, nc_for_printing)) : ext_named_context) =
  let open EConstr in
  let open Vars in
  let map_decl f d =
  ...
  let nc_for_printing =
    ...
    push_named_context_val d nc_for_printing in
  ...
  | Some id0 ->
    ...
    | RenameExistingBut f | ProgramNaming f ->
      if f id0 then
        ...
        let d = decl |> NamedDecl.of_rel_decl (fun _ -> id) |> map_decl (csubst_subst sigma subst) in
        (push_var id subst, Id.Set.add id avoid, (push_named_context_val d nc, nc_for_printing))
      else
        ...
        (push_var id0 subst, avoid, (push_named_context_val d nc, nc_for_printing))
    | FailIfConflict ->
...
let new_evar ?src ?filter ?relevance ?abstract_arguments ?candidates ?(naming = IntroAnonymous) ?typeclass_candidate
  ...
  in
  let (sign,sign_for_printing),typ',instance,subst = push_rel_context_to_named_context ~hypnaming env evd typ in
  ...
  in
  let (evd, evk) = new_pure_evar sign ~sign_for_printing evd typ' ?src ?filter ~relevance ?abstract_arguments ?candidates ?name
