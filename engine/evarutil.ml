...
let new_type_evar env evd ?src ?filter ?private_ids ?naming ?principal rigid =
  let (evd', s) = new_sort_variable rigid evd in
  let (evd', e) = new_evar env evd' ?src ?filter ?private_ids ?naming ?principal (EConstr.mkSort s) in
  evd', (e, s)

let e_new_type_evar env evdref ?src ?filter ?private_ids ?naming ?principal rigid =
  let (evd, c) = new_type_evar env !evdref ?src ?filter ?private_ids ?naming ?principal rigid in
    evdref := evd;
    c
