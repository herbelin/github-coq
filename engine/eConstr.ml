...
let decompose_lam_n_assum sigma n c =
  let open Rel.Declaration in
  if n < 0 then
    invalid_arg "decompose_lam_n_assum: integer parameter must be positive";
  let rec lamdec_rec l n c =
    if Int.equal n 0 then l,c
      ...
      | c -> failwith "decompose_lam_n_assum: not enough abstractions"
  in
...
let decompose_lam_n_decls sigma n =
  let open Rel.Declaration in
  if n < 0 then
    invalid_arg "decompose_lam_n_decls: integer parameter must be positive";
  let rec lamdec_rec l n c =
    if Int.equal n 0 then l,c
    else
      match kind sigma c with
      ...
      | Cast (c,_,_)      -> lamdec_rec l n c
      | c -> failwith "decompose_lam_n_decls: not enough abstractions"
  in
...
let decompose_prod_n_assum sigma n c =
  let open Rel.Declaration in
  if n < 0 then
    invalid_arg "decompose_prod_n_assum: integer parameter must be positive";
  let rec prodec_rec l n c =
    if Int.equal n 0 then l,c
      ...
      | c -> failwith "decompose_prod_n_assum: not enough assumptions"
  in
