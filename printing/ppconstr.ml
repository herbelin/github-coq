...
let tag_var = tag Tag.variable
  ...
  let print_hunks n pr pr_patt pr_binders (terms, termlists, binders, binderlists) unps =
    ...
    let rec aux = function
      ...
      | UnpListMetaVar (prec, sl, side) as unp :: l ->
        ...
        let pp2 = aux l in
        return unp pp1 pp2
      | UnpBinderListMetaVar (isopen, noquote, sl) as unp :: l ->
        let cl = pop bll in
        let pp2 = aux l in
        let pp1 = pr_binders (fun () -> aux sl) isopen noquote cl in
        return unp pp1 pp2
  ...
  *)
  ...
  let pr_binder_among_many noquote pr_c = function
    | CLocalAssum (nal,k,t) ->
    ...
    | CLocalDef (na,c,topt) ->
      surround (pr_lname na ++
                ...
                str" :=" ++ spc() ++ pr_c c)
    | CLocalPattern p ->
      str (if noquote then "" else "'") ++ pr_patt pr_c lsimplepatt p

  let pr_undelimited_binders sep noquote pr_c =
    prlist_with_sep sep (pr_binder_among_many noquote pr_c)

  let pr_delimited_binders kw sep noquote pr_c bl =
    let n = begin_of_binders bl in
    match bl with
      | [CLocalAssum (nal,k,t)] ->
        kw n ++ pr_binder false pr_c (nal,k,t)
      | (CLocalAssum _ | CLocalPattern _ | CLocalDef _) :: _ as bdl ->
        kw n ++ pr_undelimited_binders sep noquote pr_c bdl
      | [] -> anomaly (Pp.str "The ast is malformed, found lambda/prod without proper binders.")

  let pr_binders_gen pr_c sep is_open noquote =
    if is_open then pr_delimited_binders pr_com_at sep noquote pr_c
    else pr_undelimited_binders sep noquote pr_c
  ...
  let pr_recursive_decl pr pr_dangling kw dangling_with_for id bl annot t c =
    ...
    hov 0 (str kw ++ brk(1,2) ++ pr_id id ++ (if bl = [] then mt () else brk(1,2)) ++
      hov 0 (pr_undelimited_binders spc false (pr ltop) bl ++ annot) ++
  ...
  let pr pr sep inherited a =
    ...
    let (strm, prec) = match CAst.(a.v) with
      ...
      | CProdN (bl,a) ->
        return (
          hov 0 (
            hov 2 (pr_delimited_binders pr_forall spc false
                     (pr mt ltop) bl) ++
              str "," ++ pr spc ltop a),
          lprod
        )
      | CLambdaN (bl,a) ->
        return (
          hov 0 (
            hov 2 (pr_delimited_binders pr_fun spc false
                     (pr mt ltop) bl) ++
  ...
  let pr_binders env sigma = pr_undelimited_binders spc false (pr_expr env sigma ltop)
