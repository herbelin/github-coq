...
let pattern_strings : type c. c p -> string * string option =
  ...
  | PNUMERAL (Some n) -> "NUMERAL", Some (NumTok.Unsigned.to_string n)
  | PSTRING s -> "STRING", s
...
let extract_string diff_mode = function
  ...
  | NUMERAL n -> NumTok.Unsigned.to_string n
  | LEFTQMARK -> "?"
  | BULLET s -> s
  | QUOTATION(_,s) -> s
  | EOI -> ""
...
let match_pattern (type c) (p : c p) : t -> c =
  ...
  match p with
  | PKEYWORD s -> (function KEYWORD s' when seq s s' -> s' | NUMERAL n when seq s (NumTok.Unsigned.to_string n) -> s | _ -> err ())
  ...
  | PNUMERAL (Some n) -> let s = NumTok.Unsigned.to_string n in (function NUMERAL n' when s = NumTok.Unsigned.to_string n' -> n' | _ -> err ())
