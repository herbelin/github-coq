Fail Check fun (f:nat->nat->nat) (p:nat) (p:nat) => eq_refl : f _ 0 = f _ 1.

Goal True.
unshelve eset (g (f:nat->nat) (p:nat) (p:nat) := eq_refl : f _ = f _).
Show 1.
Show 2.
Show Existentials.
Abort.
...
(* Note: would not fail with the return clause uncommented *)
Fail Definition Painting' n p {Hp:p <= n}
 ...
 (fix aux p Hp :=
 match Hp (* in p <= _ return frame' p (leI_raise_both Hp) -> Type *) with
  | leI_refl _ => E
  | @leI_down _ p Hp => fun d =>
       {l: painting'' p (restrFrame' p d) & (aux p.+1 Hp) (d; l)}
  end) p Hp.
(* Before, was displaying terms such as: ?P@{p0:=p; Hp0:=Hp; p1:=p0; Hp1:=Hp0; p:=p1; Hp:=Hp1}
   instead of simply ?P *)
