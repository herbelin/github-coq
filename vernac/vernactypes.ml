...
module OpaqueAccess = struct
  ...
  (* Modification of sealed tables (by Require registering foreign
...
let tuple { prog; proof; sealed_access } = (prog, proof), sealed_access
let untuple ((prog, proof), sealed_access) = { prog; proof; sealed_access }

type no_state = (unit, unit, unit) state_gen
let no_state = { prog = (); proof = (); sealed_access = (); }

let ignore_state = { prog = Prog.Ignore; proof = Proof.Ignore; sealed_access = OpaqueAccess.Ignore }
...
let run (TypedVernac { spec = { prog; proof; sealed_access }; run }) (st:full_state) : full_state * _ =
  let ( * ) = combine_runners in
  let runner = Prog.runner prog * Proof.runner proof * OpaqueAccess.runner sealed_access in
...
let vtsealedaccess f = typed_vernac { ignore_state with sealed_access = Access }
    (fun {sealed_access} -> let () = f ~sealed_access in no_state)
