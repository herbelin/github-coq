...
let pr_vernac_expr v =
  ...
  | VernacToggleNotation (b,use,s) ->
    ...
    let in_use = function
      | Notation.ParsingAndPrinting -> []
      | Notation.OnlyParsing -> ["only parsing"]
      | Notation.OnlyPrinting -> ["only printing"] in
    let pr_flags = function
      | [] -> mt ()
      | l -> str "(" ++ prlist_with_sep pr_comma str l ++ str ")" in
    let pp = match s with
    | Notation.NotationRule (sc,(custom,s)) ->
    ...
    | Notation.SynDefRule qid -> pr_qualid qid ++ pr_flags (in_use use) in
