...
let rec explain_interp_error = function
  ...
  | Notation.NumeralNotationError(ctx,sigma,te) ->
      Himsg.explain_numeral_notation_error ctx sigma te
  | Typeclasses_errors.TypeClassError(env, te) ->
      Himsg.explain_typeclass_error env te
  | Implicit_quantifiers.MismatchedContextInstance(e,c,l,x) ->
      Himsg.explain_mismatched_contexts e c l x
  | InductiveError e ->
  ...
  | Logic_monad.TacticFailure e ->
      explain_interp_error e
  | _ ->
      raise CErrors.Unhandled

let _ = CErrors.register_handler explain_interp_error

let process_vernac_interp_error ?(allow_uncaught=true) (exc, info as e) =
  if not allow_uncaught && not (CErrors.handled exc) then
    let (e, info) = e in
