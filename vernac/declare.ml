...
let definition_entry_core ?(opaque=false) ?using ?(inline=false) ?feedback_id ?types
    ?(univs=(default_univ_entry,UnivNames.empty_binders)) ?(eff=Evd.empty_side_effects) ?(univsbody=Univ.ContextSet.empty) body =
...
let pure_definition_entry ?(opaque=false) ?(inline=false) ?types
    ?(univs=default_univ_entry,UnivNames.empty_binders) body =
...
let cast_opaque_proof_entry (type a b) (entry : (a, b) effect_entry) (e : a proof_entry) : b Entries.opaque_entry =
  ...
  | Some hyps -> hyps
  in
  let (body, univ_entry : b * _) = match entry with
  | PureEntry ->
    ...
    | Entries.Polymorphic_entry _ as univ_entry ->
      assert (Univ.ContextSet.is_empty uctx);
      univ_entry
    in
    body, univ_entry
  | EffectEntry -> e.proof_entry_body, fst (e.proof_entry_universes)
  in
...
let define_constant ~name ~typing_flags cd =
  ...
  let decl, unsafe, ubinders = match cd with
    ...
    | ParameterEntry (e, ubinders) ->
      ConstantEntry (Entries.ParameterEntry e), not (Lib.is_modtype_strict()), (snd (pi2 e), ubinders)
  ...
  let impl,opaque = match d with (* Fails if not well-typed *)
    ...
    | SectionLocalDef (de) ->
      ...
      let poly, type_univ_entry, binders = match de.proof_entry_universes with
        ...
        | Entries.Polymorphic_entry (_, uctx), binders -> true, Univ.ContextSet.of_context uctx, binders
      in
  ...
  let declare_variable ~pinfo ~uctx ~sec_vars ~univs =
    ...
    List.map_i (
      fun i { CInfo.name; typ; impargs } ->
        ...
        declare_assumption ~name ~scope ~hook ~impargs ~uctx (pe, ubinders)
