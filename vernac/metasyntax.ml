  ...
  with Not_found ->
    if is_active_compat se.synext_compat then begin
      ...
      Notation.declare_notation_rule ntn
        ~extra:se.synext_extra se.synext_unparsing se.synext_notgram
    end
...
let make_pp_rule level (typs,symbols) fmt =
  ...
  | Some fmt -> hunks_of_format (level, List.split typs) (symbols, parse_format fmt) in
  let trailingprec = trailing_precedence typs level symbols in
  (unp, level, trailingprec)
