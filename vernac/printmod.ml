...
let nametab_register_body mp dir (l,body) =
  ...
  match body with
    ...
    | SFBmodtype _ -> () (* TODO *)
    | SFBconst _ ->
      push (Label.to_id l) (GlobRef.ConstRef (Constant.make_canonical mp l))
    | SFBmind mib ->
      let mind = MutInd.make_canonical mp l in
      Array.iteri
        (fun i mip ->
...
let print_body is_impl extent env mp (l,body) =
  ...
  hov 2 (match body with
    ...
    | SFBmind mib ->
      match extent with
      | WithContents ->
        pr_mutual_inductive_body env (MutInd.make_canonical mp l) mib None
      | OnlyNames ->
        let keyword =
          let open Declarations in
          match mib.mind_finite with
