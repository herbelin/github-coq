...
Proof.
  ...
  exists f. intros x; specialize Hf with x. intuition. now rewrite (H0 y).
Qed.
...
Proof.
  ...
  intros A B; apply funct_choice_imp_functional_rel_reification.
...
Proof.
  ...
  { intros A' B'. apply description_rel_choice_imp_funct_choice.
    - apply FunRelReify.
...
Theorem functional_rel_reification_and_repr_fun_choice_ssi_setoid_fun_choice :
  FunctionalRelReification /\ RepresentativeFunctionalChoice <-> SetoidFunctionalChoice.
Proof.
  split; intros.
...
Proof.
  ...
  - intros (FunChoice & SetoidFunRepr & EM).
    ...
    + intros A B. apply funct_choice_imp_description, FunChoice.
...
Proof.
  ...
  - intros (FunChoice & ExtPredRepr & PI).
    apply functional_rel_reification_and_repr_fun_choice_imp_setoid_fun_choice.
    + intros A B. now apply funct_choice_imp_description.
