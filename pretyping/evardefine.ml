...
let define_pure_evar_as_product evd evk =
  ...
  let evd1,(dom,u1) =
    new_type_evar evenv evd univ_flexible_alg ~filter:(evar_filter evi) ~private_ids
  in
  let evd2,rng =
    let newenv = push_named (LocalAssum (id, dom)) evenv in
    let src = evar_source evk evd1 in
    let private_ids = Id.Set.add id private_ids in
    let filter = Filter.extend 1 (evar_filter evi) in
      if is_prop_sort (ESorts.kind evd1 s) then
       (* Impredicative product, conclusion must fall in [Prop]. *)
        new_evar newenv evd1 concl ~src ~filter ~private_ids
      else
	let status = univ_flexible_alg in
	let evd3, (rng, srng) =
          new_type_evar newenv evd1 status ~src ~filter ~private_ids
        in
...
let define_pure_evar_as_lambda env evd evk =
  ...
  let newenv = push_named (LocalAssum (id, dom)) evenv in
  let private_ids = if Name.equal (Name id) na then private_ids else Id.Set.add id private_ids in
  let filter = Filter.extend 1 (evar_filter evi) in
  let src = evar_source evk evd1 in
  let evd2,body = new_evar newenv evd1 ~src (subst1 (mkVar id) rng) ~filter ~private_ids in
