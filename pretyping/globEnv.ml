...
let new_evar env sigma ?src ?rrpat ?(naming = Namegen.IntroAnonymous) ?relevance typ =
  let (subst, _, (sign, sign_for_naming_instance)) as ext = Lazy.force env.extra in
  ...
  let (sigma, evk) = new_pure_evar ~typeclass_candidate sign ~sign_for_naming_instance sigma typ' ?src ?rrpat ~relevance ?name in
