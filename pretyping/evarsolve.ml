...
let define_evar_from_virtual_equation define_fun env evd src t_in_env ty_t_in_sign sign sign' filter inst_in_env =
  let (evd, evk) = new_pure_evar sign ~sign_for_printing:sign' evd ty_t_in_sign ~filter ~src in
...
let materialize_evar define_fun env evd k (evk1,args1) ty_in_env =
  ...
  let (evd, ev2_in_sign) =
    new_pure_evar sign2 ~sign_for_printing:sign2' evd ev2ty_in_sign ~filter:filter2 ~src in
