...
let occur_glob_constr id =
  let rec occur barred acc c = CAst.with_val (function
    ...
    | _ ->
       ...
       fold_glob_constr_with_binders g f barred acc c) c in
  occur false false

let free_glob_vars =
  let rec vars bound vs c = CAst.with_val (function
    | GVar id' -> if Id.Set.mem id' bound then vs else Id.Set.add id' vs
    | _ -> fold_glob_constr_with_binders Id.Set.add vars bound vs c) c in
  fun rt ->
