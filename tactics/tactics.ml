...
let prepare_intros_opt with_evars dft destopt ipat =
  ...
  | None ->
    let pat = IntroNaming dft in make_naming Id.Set.empty [] pat, None, pat
  | Some {CAst.loc;v=(IntroNaming _ as ipat)} ->
    (* "apply ... in H as id" needs to use id and H is kept iff id<>H *)
    make_naming Id.Set.empty [] ipat, loc, ipat
  | Some {CAst.loc;v=(IntroAction _ as ipat)} ->
    (* "apply ... in H as pat" reuses so that old H is always cleared *)
    make_naming Id.Set.empty [] (IntroNaming dft), loc, ipat
