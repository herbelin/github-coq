...
type tactic_error =
  ...
  | ClearDependency of env * evar_map * Id.t * Evarutil.clear_dependency_error * GlobRef.t option
  ...
  | SchemeDontApply
  | UnableToApplyLemma of env * evar_map * constr * clausenv
  | NeedFullyAppliedArgument
  | NotRightNumberConstructors of int
  | NotEnoughConstructors
  | ConstructorNumberedFromOne
  | NoConstructors
  | UnexpectedExtraPattern of (bool * int) option * delayed_open_constr intro_pattern_expr
  ...
  | IllegalClause
  | IllegalNonAtomicClause
  | OccurrencesNotSupported
  | ReplacementIllTyped
...
let explain_tactic_error = function
  ...
  | UnableToApplyLemma (env,sigma,thm,innerclause) ->
      ...
      quote (Printer.pr_leconstr_env innerclause.env innerclause.evd (clenv_type innerclause)) ++
      str "."
  | NeedFullyAppliedArgument ->
  ...
  | IllegalClause ->
      str "\"at\" clause not supported in presence of an occurrence clause."
  | IllegalNonAtomicClause ->
      str "\"at\" clause not supported in presence of a non atomic \"in\" clause."
  | OccurrencesNotSupported ->
      str "Occurrences not supported for this reduction tactic."
  | ReplacementIllTyped ->
...
let error_clear_dependency env sigma id err inglobal =
  error (ClearDependency (env,sigma,id,err,inglobal))
...
let apply_in_once_main flags innerclause env sigma (loc,d,lbind) =
  ...
  let rec aux clause =
    ...
    with NotExtensibleClause ->
      match e with
      | UnableToApply -> error ?loc (UnableToApplyLemma (env,sigma,thm,innerclause))
      | _ -> Exninfo.iraise e'
  in
