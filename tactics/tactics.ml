...
exception UnsupportedOverClause of bool
...
let tactic_interp_error_handler = function
  ...
  | UnsupportedOverClause b ->
      str (if b then "'over' clause not supported here."
           else "'over' clause not yet supported here.")
...
let induction_destruct isrec with_evars (lc,elim) =
  ...
  | [c,(eqname,names as allnames),(cls,over)] ->
    ...
    | Some elim when is_functional_induction elim gl ->
      ...
      if not (List.is_empty over) then error (UnsupportedOverClause true);
    ...
    | Some elim ->
      ...
      let newlc =
        List.map (fun (x,(eqn,names),(cls,over)) ->
          ...
          if over != [] then error (UnsupportedOverClause false);
