...
let make_evar_clause env sigma ?len t =
  ...
  let rec clrec (sigma, holes) inst n t =
    ...
    | Prod (na, t1, t2) ->
      ...
      let inst, ctx, sign_for_naming_instance, args, subst = match inst with
      | None ->
        (* Dummy type *)
        let hypnaming = VarSet.variables (Global.env ()) in
        let (ctx, sign_for_naming_instance), _, args, subst = push_rel_context_to_named_context ~hypnaming env sigma mkProp in
        Some (ctx, args, subst), ctx, sign_for_naming_instance, args, subst
      | Some (ctx, args, subst) -> inst, ctx, ctx, args, subst
      in
      let (sigma, evk) = new_pure_evar ~typeclass_candidate:false ctx ~sign_for_naming_instance sigma
