...
struct
  ...
  struct
    ...
    let get_pure_id = function
      | LocalAssum (id,_) -> TrackedId.id id
      | LocalDef (id,_,_) -> TrackedId.id id
    ...
    let equal eq decl1 decl2 =
      ...
      | LocalAssum (id1, ty1), LocalAssum (id2, ty2) ->
          TrackedId.equal id1 id2 && eq ty1 ty2
      | LocalDef (id1, v1, ty1), LocalDef (id2, v2, ty2) ->
          TrackedId.equal id1 id2 && eq v1 v2 && eq ty1 ty2
      | _ ->
          false
  ...
  let rec lookup id = function
    | decl :: _ when Id.equal id (TrackedId.id (Declaration.get_id decl)) -> decl
  ...
  let to_instance mk l =
    let filter = function
      | Declaration.LocalAssum (id, _) -> Some (mk (TrackedId.id id))
      | _ -> None
    in
