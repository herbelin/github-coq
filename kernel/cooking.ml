...
let expand_constr cache modlist c =
  ...
  let rec substrec c =
    match kind c with
      ...
      | Proj (p, c') ->
        let map cst npars =
          let newpars = (Mindmap.find cst (snd modlist)).abstr_inst in
...
(** Absorb in a named context in the transformation which turns a
...
let make_cooking_info expand_info hyps uctx =
  ...
  let abstr_info = abstract_context expand_info abstr_info hyps in
...
type 'opaque result = {
  ...
  cook_univcontext : UContext.t list option;
...
let lift_private_univs info = function
  ...
  | Opaqueproof.PrivatePolymorphic (univs, ctx) ->
    ...
    let info, auctx = lift_poly_univs info (AbstractContext.make names cstrs) in
...
let cook_opaque_proofterm infos c =
  let fold info (c, priv) =
    let info, priv = lift_private_univs info priv in
...
let cook_constant { from = cb; info } =
  let cache = RefTable.create 13 in
  (* Adjust the info so that it gets meaningful under the block of quantified universe binders *)
  ...
  let mind_variance, mind_sec_variance =
    ...
    | Some variance, Some sec_variance ->
      let sec_variance, newvariance =
        Array.chop (Array.length sec_variance - AbstractContext.size info.abstr_info.abstr_uctx)
          sec_variance
      in
