...
let empty_named_context_val = {
  ...
  env_named_private = Id.Set.empty;
}
...
type private_flag = bool   (* true = var name generated by Coq *)

let push_named_context_val_val d rval isprivate ctxt =
(*   assert (not (Id.Map.mem (NamedDecl.get_id d) ctxt.env_named_map)); *)
  let id = NamedDecl.get_pure_id d in
  let private_ids = if isprivate then Id.Set.add id ctxt.env_named_private else ctxt.env_named_private in
  {
    env_named_ctx = Context.Named.add d ctxt.env_named_ctx;
    env_named_map = Id.Map.add (NamedDecl.get_pure_id d) (d, rval) ctxt.env_named_map;
    env_named_private = private_ids;
  }

let push_named_context_val d isprivate ctxt =
  push_named_context_val_val d (ref VKnone) isprivate ctxt

let match_named_context_val c = match c.env_named_ctx with
| [] -> None
| decl :: ctx ->
  let (_, v) = Id.Map.find (NamedDecl.get_pure_id decl) c.env_named_map in
  let map = Id.Map.remove (NamedDecl.get_pure_id decl) c.env_named_map in
  let priv = Id.Set.remove (NamedDecl.get_pure_id decl) c.env_named_private in
  let cval = { env_named_ctx = ctx; env_named_map = map ; env_named_private = priv } in
  Some (decl, v, Id.Set.mem (NamedDecl.get_pure_id decl) c.env_named_private, cval)
...
let map_named_val f ctxt =
  ...
  let fold accu d =
    ...
    let accu =
      if d == d' then accu
      else Id.Map.modify (get_pure_id d) (fun _ (_, v) -> (d', v)) accu
    in
    (accu, d')
  in
  let map, ctx = List.fold_left_map fold ctxt.env_named_map ctxt.env_named_ctx in
  if map == ctxt.env_named_map then ctxt
  else { env_named_ctx = ctx; env_named_map = map; env_named_private = ctxt.env_named_private }

let set_named_context_private sign private_ids =
  { sign with env_named_private = private_ids }

let push_named d isprivate env =
  {env with env_named_context = push_named_context_val d isprivate env.env_named_context}
