...
type table_val = (fconstr * gfix_info option * bool array, Empty.t, UVars.Instance.t * bool * rewrite_rule list) constant_def
...
let rec to_constr (lfts, usubst as ulfts) v =
  ...
  match v.term with
    ...
    | FCaseT (ci, u, pms, p, c, ve, env) ->
      to_constr_case ulfts ci u pms p NoInvert (to_constr ulfts c) ve env
    ...
    | FFix (_, e, Some (true,gfix)) ->
      if is_subs_id (fst e) && is_lift_id lfts then
        subst_instance_constr (usubst_instance ulfts (snd e)) gfix
      else
    ...
    | FFix ((op,(lna,tys,bds)) as fx, e, gfix) ->
      ...
      if is_subs_id (fst e) && is_lift_id lfts then
        subst_instance_constr (usubst_instance ulfts (snd e)) fix
      else
