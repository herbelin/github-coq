...
let build_constant_declaration env result =
  ...
  let hyps, def =
    let context_ids = List.map ShortNamedDecl.get_id (section_context env) in
  ...
  let hyps = List.filter (fun d -> Id.Set.mem (ShortNamedDecl.get_id d) hyps) (Environ.section_context env) in
  let tps =
