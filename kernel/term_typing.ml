...
type typing_context =
| MonoTyCtx of Environ.env * unsafe_type_judgment * Cset.t * Stateid.t option
| PolyTyCtx of Environ.env * unsafe_type_judgment * universe_level_subst * AbstractContext.t * Cset.t * Stateid.t option
...
let infer_declaration env (dcl : constant_entry) =
  ...
  | ParameterEntry entry ->
    ...
    let t = Vars.subst_univs_level_constr usubst j.uj_val in
    {
      ...
      cook_univcontext = entry.parameter_entry_secunivctx;
      cook_flags = Environ.typing_flags env;
    }
  ...
  | DefinitionEntry c ->
        ...
        feedback_completion_typecheck feedback_id;
      {
        ...
        cook_univcontext = c.const_entry_secunivctx;
        cook_flags = Environ.typing_flags env;
      }
...
let infer_opaque env = function
  | ({ opaque_entry_type = typ;
      ...
      let def = OpaqueDef () in
      {
        ...
        cook_univcontext = Some c.opaque_entry_secunivctx;
        cook_flags = Environ.typing_flags env;
      }, c.opaque_entry_secunivctx, context
...
let check_section_variables env declared_set typ body =
  ...
  let ids_def = global_section_set env body in
  let inferred_set = Cset.union ids_typ ids_def in
  if not (Cset.subset inferred_set declared_set) then
    let l = Cset.elements (Cset.diff inferred_set declared_set) in
    let n = List.length l in
    let pr cst = Id.print (Constant.basename cst) in
    let declared_vars = Pp.pr_sequence pr (Cset.elements declared_set) in
    let inferred_vars = Pp.pr_sequence pr (Cset.elements inferred_set) in
    let missing_vars  = Pp.pr_sequence pr (List.rev l) in
...
let build_constant_declaration env sec_univ result =
  let open Cooking in
  ...
  let hyps, def =
    ...
    | None ->
      if List.is_empty sec_univ then
        (* No section: no need to check *)
        Cset.empty, def
      else
        ...
        | Undef _ | Primitive _ -> Cset.empty
        | Def cs -> global_section_set env cs
        | OpaqueDef _ ->
          (* Opaque definitions always come with their section variables *)
          assert false
        in
        Cset.union ids_typ ids_def, def
    | Some declared ->
...
let translate_constant env _kn sec_univ ce =
  build_constant_declaration env sec_univ
    (infer_declaration env ce)

let translate_opaque env _kn ce =
  let def, sec_univ, ctx = infer_opaque env ce in
  build_constant_declaration env sec_univ def, ctx
...
let translate_recipe env _kn r =
  ...
  let hyps = Cset.elements (Option.get result.cook_context) in
  { const_hyps = hyps;
    const_secunivctx = Option.get result.cook_univcontext;
