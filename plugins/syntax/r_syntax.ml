...
let r_of_rawnum ?loc n =
  let n,e = NumTok.Signed.to_bigint_fraction n in
  if Bigint.is_strictly_neg n then raise Notation.NoPrimNumberInterpretation;
  let izr z =
     ...
     | GApp (i, [l]), GApp (i', [r])
        ...
        | GApp (p, [t; e]) when is_gr p glob_pow_pos ->
           ...
           if not (Bigint.(equal t (of_int 10))) then
             ...
             NumTok.Signed.of_bigint_fraction i e
        | _ -> raise Non_closed_number
        end
     | _ -> raise Non_closed_number
     end
