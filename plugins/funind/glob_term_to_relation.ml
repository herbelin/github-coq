...
(*   in *)
  ...
  try
    with_full_print
      (Flags.silently (ComInductive.do_mutual_inductive (Global.env()) ~template:None None rel_inds (Flags.is_universe_polymorphism ()) false false ~uniform:ComInductive.NonUniformParameters))
      Declarations.Finite
  with
