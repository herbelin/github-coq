  ...
  match cb.const_body with
    | Def lbody ->
        (match EConstr.kind sg (get_body lbody) with
          ...
          | _ -> raise Impossible)
    | Undef _ | SealedDef _ | Primitive _ | Symbol _ -> raise Impossible
...
let mono_environment ~sealed_access refs mpl =
  Visit.reset ();
  ...
  List.rev_map
    (fun (mp,struc) ->
      mp, extract_structure sealed_access env mp no_delta ~all:(Visit.needed_mp_all mp) struc)
    l
...
let simple_extraction ~sealed_access r =
  match locate_ref [r] with
  | ([], [mp]) as p -> full_extr sealed_access None p
  | [r],[] ->
      init false false;
      let struc = optimize_struct ([r],[]) (mono_environment ~sealed_access [r] []) in
...
let extraction_library ~sealed_access is_rec CAst.{loc;v=m} =
  init true true;
  let dir_m =
  ...
  let select l (mp,struc) =
    ...
    then (mp, extract_structure sealed_access env mp no_delta ~all:true struc) :: l
    else l
  in
...
let structure_for_compute ~sealed_access env sg c =
  ...
  let struc = optimize_struct (refs,[]) (mono_environment ~sealed_access refs []) in
