...
let set_maximality fail na i imps b =
  ...
  b || (
    let is_set (_,x) = match x with None -> false | _ -> true in
...
let binding_kind_of_status = function
  | _, Some (_, false, _) -> NonMaxImplicit
...
let match_argument i imp pos = match imp, pos with
  ...
  | ((_,Some n,_),_), ExplByPos n' -> Int.equal n n'
...
let discharge_implicits (flags,l) =
  let l' =
...
let rebuild_implicits (flags,l) =
  let process_one (gr,_,userimplsize,oldimpls) =
    let newlocal = isVarRef gr && Lib.is_in_section gr in
...
let classify_implicits (flags,l) =
...
let declare_var_implicits id ~impl ~impargs =
  ...
  Lib.add_leaf (inImplicits (flags,[imps]))
...
let declare_mib_implicits mind ~impargs =
  ...
  Lib.add_leaf (inImplicits (flags,List.flatten imps))
...
let set_auto_implicits local ref =
  ...
  Lib.add_leaf (inImplicits (flags,[ref,local,0,imps]))
...
let set_manual_implicits local ref l =
  ...
  Lib.add_leaf (inImplicits (flags,[ref,local,userimplsize,l']))
