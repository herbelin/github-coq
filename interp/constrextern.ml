  ...
  match snd ntn,List.map destprim l with
    ...
    | _ ->
        ...
        | (InConstrEntrySomeLevel,[Terminal x]), [] ->
           begin match NumTok.Signed.of_string x with
           | Some n -> mkprim (loc, Numeral n)
...
let extern_float f scopes =
  if Float64.is_nan f then CRef(q_nan (), None)
    ...
    match NumTok.Signed.of_string s with
    | None -> assert false
    | Some n ->
...
let rec extern inctx scopes vars r =
  ...
  | GInt i ->
     extern_prim_token_delimiter_if_required
       (Numeral (NumTok.Signed.of_int (NumTok.SignedNat.of_string (Uint63.to_string i))))
